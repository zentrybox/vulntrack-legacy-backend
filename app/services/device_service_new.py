...
class DeviceService:
    ...
    def get_device_by_serial_number(self, serial_number: str) -> Optional[Device]:
        ...
        return self.db.query(Device).filter(Device.serial_number == serial_number.upper()).first()
    ...
    def get_devices(
        ...
        return query.offset(skip).limit(limit).order_by(Device.created_at.desc()).all()
    ...
    def get_devices_by_version(self, version: str) -> List[Device]:
        ...
        return self.db.query(Device).filter(Device.version == version).order_by(Device.name).all()

    def get_devices_by_brand_and_model(self, brand: str, model: Optional[str] = None) -> List[Device]:
    ...
    def update_device(self, device_id: uuid.UUID, device_data: DeviceUpdate) -> Optional[Device]:
        """Update a device"""
    ...
    def get_version_summary(self) -> List[dict]:
        ...
        result = self.db.query(
            ...
            Device.version, Device.brand
        ).order_by(
            Device.brand, Device.version
        ).all()
