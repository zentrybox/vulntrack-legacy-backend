...
def get_devices(
    ...
    devices = service.get_devices(
        skip=skip, limit=limit, is_active=is_active, user_id=user_id
    )
...
def update_device(
    device_id: uuid.UUID, device_data: DeviceUpdate, db: Session = Depends(get_db)
):
    """Update a device"""
    service = DeviceService(db)
    ...
    if (
        ...
        and device_data.serial_number != existing_device.serial_number
    ):
...
def deactivate_device(device_id: uuid.UUID, db: Session = Depends(get_db)):
...
def get_version_summary(db: Session = Depends(get_db)):
