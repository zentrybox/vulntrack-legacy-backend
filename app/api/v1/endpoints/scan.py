...
class ScanBatchRequest(BaseModel):
    device_ids: list[str] = Field(
        ...
        example=["99d8b93e-14eb-4000-9681-b0ee16ae6fef", "b1b2b3b4-5678-1234-5678-abcdefabcdef"]
    )

# --- POST / ---
@router.post("/", status_code=status.HTTP_202_ACCEPTED, summary="Disparar escaneo para un dispositivo", response_description="Scan iniciado")
...
# --- POST /batch ---
@router.post("/batch", status_code=status.HTTP_202_ACCEPTED, summary="Disparar escaneos batch", response_description="Batch de scans iniciado")
async def trigger_batch_scan(body: ScanBatchRequest) -> Dict[str, Any]:
    device_ids = body.device_ids
    if not device_ids or not isinstance(device_ids, list):
        raise HTTPException(status_code=400, detail="device_ids must be a list of UUIDs")
    ...
    return {"job_id": job_id, "message": f"{len(initiated_scans)} scans initiated.", "initiated_scans": initiated_scans}
