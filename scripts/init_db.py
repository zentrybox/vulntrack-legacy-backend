...
def create_tables():
    ...
    try:
        ...
        with engine.connect() as conn:
            result = conn.execute(text("SELECT name FROM sqlite_master WHERE type='table'" if "sqlite" in database_url else "SELECT tablename FROM pg_tables WHERE schemaname='public'"))
